...
class sale_order_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        def get_string_discount(res_dict):
        ...
        if product:
            ...
            if(len(pricelists) > 0 and pricelists[0]['visible_discount'] and list_price[pricelist] != 0):
                ...
                #manda anche il prezzo extra
                if product.price_extra:
                  result['extra_price_variant'] = product.price_extra  
                  result['price_unit'] = new_list_price
                else:
                  result['price_unit'] = new_list_price
                # import pdb;pdb.set_trace()                 
                item_String_Discount = get_string_discount(list_price)
                    
                if result.get('string_discount', False):                  
                    result['string_discount'] = item_String_Discount
                    
                else:
                    result.update({'string_discount':item_String_Discount})
                sconto = self.Calcolo_Sconto(cr, uid, ids, item_String_Discount, context=None)
                if sconto['value']['discount'] == 100:
                    # prezzo netto senza sconti
                    result['price_unit'] = get_prezzo_netto(list_price)
                    result['discount'] = 0.0
                    result.update({'string_discount':''})
                else:
